...
(function () {
  ...
  document.addEventListener("DOMContentLoaded", async function () {
    ...
    async function loadServices() {
      ...
      if (res.error) {
        svcTbody.innerHTML =
          '<tr><td colspan="5">' + (P ? P.escapeHtml(res.error.message) : res.error.message) + "</td></tr>";
        return;
      }
      if (!res.data || !res.data.length) {
        svcTbody.innerHTML =
          '<tr><td colspan="5" style="color: var(--color-text-muted)">No services yet. Add one below.</td></tr>';
        return;
      }
      ...
      res.data.forEach(function (s) {
        ...
        tr.innerHTML =
          ...
          "</td><td>$" +
          (P ? P.escapeHtml(price) : price) +
          '</td><td><span class="badge ' +
          (s.is_active ? "badge--verified" : "badge--pending") +
          '">' +
          (s.is_active ? "Active" : "Hidden") +
          '</span></td><td><button type="button" class="btn btn--secondary btn--sm js-service-toggle" style="padding:0.35rem 0.65rem;font-size:0.75rem">' +
